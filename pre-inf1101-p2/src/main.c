...
static int run_interpreter(index_t *idx, list_t *piped_input) {
    ...
    while (1) {
        ...
        if (piped_input) {
            ...
            printf("%s\n", input); // simulate actual input
        } else if (fgets(input, LINE_MAX, stdin) == NULL) {
...
static index_t *build_index(list_t *fpaths) {
    ...
    index_t *idx = index_create((cmp_fn) strcmp, (hash64_fn) hash_string_fnv1a64);
    if (idx == NULL) {
...
static list_t *read_piped_lines() {
     ...
     * - minimum length 1
     * - split on newlines
     * - include only visible characters
     */
    int status = tokenize_file(stdin, piped, 1, is_newline, isgraph, NULL);
    ...
    if (status != 0) {
        pr_error("Failed to read stdin: %s\n", strerror(errno));
